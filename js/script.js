...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initSearchAndFilter();
    renderProducts();
    // renderTestimonials(); // Digantikan sistem ulasan interaktif baru — lihat js/reviews.js
    initSmoothScroll();
    initCart();
    initAudioPlayer();
    startSoldCountSync();
...
function saveCart() {
    // User sudah login via Google -> keranjang WAJIB disimpan ke Firebase
    // Realtime Database di /carts/{uid}, bukan localStorage lagi.
    if (window.currentUser && window.currentUser.uid && typeof window.syncCartToCloud === 'function') {
        const nameInput = document.getElementById('buyerName');
        const buyerName = nameInput ? nameInput.value.trim() : '';
        window.syncCartToCloud(cart, buyerName);
        return;
    }
    // User belum login -> simpan sementara di localStorage (guest cart),
    // nanti otomatis dipindah/sync ke Firebase begitu berhasil login (lihat js/auth-engine.js).
    localStorage.setItem('adiprmx_cart', JSON.stringify(cart));
}

// Dipanggil oleh js/auth-engine.js setiap kali data keranjang cloud (/carts/{uid})
// berubah (login pertama, ganti akun Google, atau update realtime dari device lain).
window.setCartFromCloud = function (cloudItems) {
    cart = Array.isArray(cloudItems) ? cloudItems : [];
    updateCartUI();
};
...
// Simpan "Nama Pemesan" ke cloud cart (/carts/{uid}) setiap kali user selesai
// mengetik/mengubahnya, supaya nama yang sama otomatis muncul lagi lain kali
// login dengan akun Google yang sama (di device apa pun).
document.addEventListener('DOMContentLoaded', () => {
    const nameInputEl = document.getElementById('buyerName');
    if (nameInputEl) {
        nameInputEl.addEventListener('blur', () => {
            if (window.currentUser && window.currentUser.uid) saveCart();
        });
    }
});
...
function checkout() {
    ...
    const buyerNote = buyerNoteInput ? buyerNoteInput.value.trim() : '';

    if (!buyerName) {
        showToast('Lengkapi Nama Pemesan terlebih dahulu!');
        return;
    }
    ...
    // Clear cart after checkout (otomatis ikut mengosongkan /carts/{uid} di
    // Firebase kalau user sedang login, lihat saveCart())
    cart = [];
    saveCart();
    updateCartUI();
    closeCart();
