...
(function () {
    ...
    // ----------------------------------------
    // DASHBOARD ANALYTICS: hitung ringkasan (profit, jumlah order, rata-rata
    // harga) dari data /orders + /transactions yang sudah realtime ter-listen
    // di atas, lalu broadcast lewat CustomEvent supaya admin.html (Metrics
    // Cards + Revenue Chart) bisa dengar tanpa perlu tahu detail internal
    // modul ini. Tidak pernah menyentuh/merubah data, cuma baca & hitung.
    // ----------------------------------------
    function broadcastStats() {
        const full = buildUnifiedList();
        let totalProfit = 0;
        let lunasCount = 0;
        const monthly = {};      // 'YYYY-MM' -> total Rp (LUNAS saja)
        const monthlyCount = {}; // 'YYYY-MM' -> jumlah transaksi (semua status)
        ...
        window.dispatchEvent(new CustomEvent('adiprmx:stats-update', {
            detail: {
                totalOrders: full.length,
                lunasCount: lunasCount,
                totalProfit: totalProfit,
                avgPrice: lunasCount ? Math.round(totalProfit / lunasCount) : 0,
                monthly: monthly,
